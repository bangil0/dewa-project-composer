...
class mhs extends MX_Controller {
    ...
    public function getdatatables(){
        if($this->isadmin()==1):
            $this->datatables->select('nim,Nama,')
                            ->from('mhs');
            ...
            $this->datatables->unset_column('nim');

        else:
            $this->datatables->select('nim,Nama,')
                            ->from('mhs');
            $this->datatables->add_column('edit',"<div class='btn-group'>
                <a data-toggle='modal' href='#modal-id' data-load-remote='".base_url('mhs/getone/$1/')."' data-remote-target='#modal-id .modal-body' class='btn btn-info btn-xs'><i class='fa fa-info-circle'></i> </a></div>" , 'id');
            $this->datatables->unset_column('nim');
        endif;
    ...
    public function get($nim=null){
        if($nim!==null){
            echo json_encode($this->mhsdb->get_one($nim));
        }
    }
    function tables(){
    ...
    public function submit(){
        if ($this->input->post('ajax')){
          if ($this->input->post('nim')){
            $this->mhsdb->update($this->input->post('nim'));
          }else{
            //$this->mhsdb->save();
            $this->mhsdb->saveas();
          }
    ...
    public function delete_detail(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['nim'])){
                $this->mhsdb->upddel_detail($this->input->post('nim'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
     ...
     public function delete_detailxx(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['nim'])){
                $this->mhsdb->delete_detail($this->input->post('nim'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
            }else {
                $this->session->set_flashdata('notif', 'Failed! No Data Deleted');
            }

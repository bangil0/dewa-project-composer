...
class mhsmaster extends MX_Controller {

    function __construct() {
        parent::__construct();
          
        //Load IgnitedDatatables Library
        $this->load->model('mhsmaster_model','mhsmasterdb',TRUE);
        $this->session->set_userdata('lihat','mhsmaster');
    ...
    function __getnewfaktur(){
        ...
        $null=$this->mhsmasterdb->ceknomornull();
        // print_r($null);
        if($null!=null||!empty($null)){
            ...
            $faktur=$this->mhsmasterdb->genfaktur();
    ...
    public function getdatatables(){
        if($this->isadmin()==1):
            ...
            $this->datatables->add_column('edit',"<div class='btn-group'>
                <a data-toggle='modal' href='#modal-id' data-load-remote='".base_url('mhsmaster/getone/$1/')."' data-remote-target='#modal-id .modal-body' class='btn btn-info btn-xs'><i class='fa fa-info-circle'></i> </a>
    ...
    function enkrip(){
        ...
        // echo $this->session->userdata('mhsmaster');
    }
    function isadmin(){
    ...
    function forms(){   
        ...
        $this->load->view('mhsmaster_form_inside');
           
    }

    public function get($id=null){
        if($id!==null){
            echo json_encode($this->mhsmasterdb->get_one($id));
        }
    }
    function tables(){
        $this->load->view('mhsmaster_data');
    }

    function getone($id=null){
        if($id!==null){
            $data=$this->mhsmasterdb->get_one($id);
            $jml=count($data);
            // print_r($jml);
            // print_r($data);
            $div='';
    ...
    public function submit(){
        if ($this->input->post('ajax')){
          if ($this->input->post('id')){
            $this->mhsmasterdb->update($this->input->post('id'));
          }else{
            //$this->mhsmasterdb->save();
            $this->mhsmasterdb->saveas();
          }
    ...
    public function delete(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id'])){
                $this->mhsmasterdb->delete($this->input->post('id'));
    ...
    public function delete_detail(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id'])){
                $this->mhsmasterdb->upddel_detail($this->input->post('id'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
     ...
     public function delete_detailxx(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id'])){
                $this->mhsmasterdb->delete_detail($this->input->post('id'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
            }else {
                $this->session->set_flashdata('notif', 'Failed! No Data Deleted');
            }
    ...
    private function gen_faktur(){
        $last=$this->mhsmasterdb->get_last_pt();
        // print_r($last);
        if(!empty($last)):

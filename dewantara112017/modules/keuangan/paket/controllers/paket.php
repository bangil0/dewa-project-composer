...
class paket extends MX_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('paket_model','paketdb',TRUE);
        $this->session->set_userdata('lihat','paket');
    ...
    public function index() {
        $this->template->set_title('Kelola Paket');
        $this->template->add_js('var baseurl="'.base_url().'paket/";','embed');  
        $this->template->load_view('paket_view',array(
            'view'=>'',
            'title'=>'Kelola Data Paket',
            'subtitle'=>'Pengelolaan Paket',
            'breadcrumb'=>array(
            'Paket'),
        ));
    }
    ...
    function __getnewfaktur(){
        ...
        $null=$this->paketdb->ceknomornull();
        // print_r($null);
    ...
    public function getdatatables(){
        if($this->isadmin()==1):
            $this->datatables->select('id,kodepaket,namapaket,isactive,isdeleted,datedeleted,userid,datetime,')
                            ->from('paket');
            $this->datatables->add_column('edit',"<div class='btn-group'>
                <a data-toggle='modal' href='#modal-id' data-load-remote='".base_url('paket/getone/$1/')."' data-remote-target='#modal-id .modal-body' class='btn btn-info btn-xs'><i class='fa fa-info-circle'></i> </a>
            ...
            $this->datatables->select('id,kodepaket,namapaket,isactive,isdeleted,datedeleted,userid,datetime,')
                            ->from('paket');
            $this->datatables->add_column('edit',"<div class='btn-group'>
                <a data-toggle='modal' href='#modal-id' data-load-remote='".base_url('paket/getone/$1/')."' data-remote-target='#modal-id .modal-body' class='btn btn-info btn-xs'><i class='fa fa-info-circle'></i> </a></div>" , 'id');
            $this->datatables->unset_column('id');
        endif;
    ...
    function forms(){   
        ...
        $this->load->view('paket_form_inside');
           
    }

    public function get($id=null){
        if($id!==null){
            echo json_encode($this->paketdb->get_one($id));
        }
    }
    function tables(){
        $this->load->view('paket_data');
    }

    function getone($id=null){
        if($id!==null){
            $data=$this->paketdb->get_one($id);
            $jml=count($data);
            // print_r($jml);
            // print_r($data);
            $div='';
    ...
    public function submit(){
        if ($this->input->post('ajax')){
          if ($this->input->post('id')){
            $this->paketdb->update($this->input->post('id'));
          }else{
            //$this->paketdb->save();
            $this->paketdb->saveas();
          }
    ...
    public function delete(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id'])){
                $this->paketdb->delete($this->input->post('id'));
    ...
    public function delete_detail(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id'])){
                $this->paketdb->upddel_detail($this->input->post('id'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
     ...
     public function delete_detailxx(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id'])){
                $this->paketdb->delete_detail($this->input->post('id'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
            }else {
                $this->session->set_flashdata('notif', 'Failed! No Data Deleted');
            }

...
class mhs extends MX_Controller {
    ...
    public function get($id=null){
        if($id!==null){
            echo json_encode($this->mhsdb->get_one($id));
        }
    }
    function tables(){
    ...
    public function submit(){
        if ($this->input->post('ajax')){
          if ($this->input->post('id')){
            $this->mhsdb->update($this->input->post('id'));
          }else{
            //$this->mhsdb->save();
            $this->mhsdb->saveas();
          }
    ...
    public function delete_detail(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id'])){
                $this->mhsdb->upddel_detail($this->input->post('id'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
     ...
     public function delete_detailxx(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id'])){
                $this->mhsdb->delete_detail($this->input->post('id'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
            }else {
                $this->session->set_flashdata('notif', 'Failed! No Data Deleted');
            }

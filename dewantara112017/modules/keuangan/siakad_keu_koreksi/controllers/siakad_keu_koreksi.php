...
class siakad_keu_koreksi extends MX_Controller {

    function __construct() {
        parent::__construct();
          
        //Load IgnitedDatatables Library
        $this->load->model('siakad_keu_koreksi_model','siakad_keu_koreksidb',TRUE);
        $this->session->set_userdata('lihat','siakad_keu_koreksi');
    ...
    public function index() {
        $this->template->set_title('Kelola Siakad_keu_koreksi');
        $this->template->add_js('var baseurl="'.base_url().'siakad_keu_koreksi/";','embed');  
        $this->template->load_view('siakad_keu_koreksi_view',array(
            'view'=>'',
            'title'=>'Kelola Data Siakad_keu_koreksi',
            'subtitle'=>'Pengelolaan Siakad_keu_koreksi',
            'breadcrumb'=>array(
            'Siakad_keu_koreksi'),
        ));
    }
    ...
    function __getnewfaktur(){
        ...
        $null=$this->siakad_keu_koreksidb->ceknomornull();
        // print_r($null);
        if($null!=null||!empty($null)){
            ...
            $faktur=$this->siakad_keu_koreksidb->genfaktur();
    ...
    public function getdatatables(){
        if($this->isadmin()==1):
            $this->datatables->select('id_sikad_keu_koreksi,jns_koreksi,nim_mhs,tgl_koreksi,nm_koreksi,nominal_koreksi,')
                            ->from('siakad_keu_koreksi');
            $this->datatables->add_column('edit',"<div class='btn-group'>
                <a data-toggle='modal' href='#modal-id' data-load-remote='".base_url('siakad_keu_koreksi/getone/$1/')."' data-remote-target='#modal-id .modal-body' class='btn btn-info btn-xs'><i class='fa fa-info-circle'></i> </a>

                <a href='#outside' data-toggle='tooltip' data-placement='top' title='Edit' class='edit btn btn-xs btn-success' id='$1'><i class='glyphicon glyphicon-edit'></i></a>
                <button data-toggle='tooltip' data-placement='top' title='Hapus' class='delete btn btn-xs btn-danger' id='$1'><i class='glyphicon glyphicon-remove'></i></button>
                </div>" , 'id_sikad_keu_koreksi');
            $this->datatables->unset_column('id_sikad_keu_koreksi');

        else:
            $this->datatables->select('id_sikad_keu_koreksi,jns_koreksi,nim_mhs,tgl_koreksi,nm_koreksi,nominal_koreksi,')
                            ->from('siakad_keu_koreksi');
            $this->datatables->add_column('edit',"<div class='btn-group'>
                <a data-toggle='modal' href='#modal-id' data-load-remote='".base_url('siakad_keu_koreksi/getone/$1/')."' data-remote-target='#modal-id .modal-body' class='btn btn-info btn-xs'><i class='fa fa-info-circle'></i> </a></div>" , 'id_sikad_keu_koreksi');
            $this->datatables->unset_column('id_sikad_keu_koreksi');
        endif;
    ...
    function enkrip(){
        ...
        // echo $this->session->userdata('siakad_keu_koreksi');
    }
    function isadmin(){
    ...
    function forms(){   
        ...
        $this->load->view('siakad_keu_koreksi_form_inside');
           
    }

    public function get($id_sikad_keu_koreksi=null){
        if($id_sikad_keu_koreksi!==null){
            echo json_encode($this->siakad_keu_koreksidb->get_one($id_sikad_keu_koreksi));
        }
    }
    function tables(){
        $this->load->view('siakad_keu_koreksi_data');
    }

    function getone($id=null){
        if($id!==null){
            $data=$this->siakad_keu_koreksidb->get_one($id);
            $jml=count($data);
            // print_r($jml);
            // print_r($data);
            $div='';
    ...
    public function submit(){
        if ($this->input->post('ajax')){
          if ($this->input->post('id_sikad_keu_koreksi')){
            $this->siakad_keu_koreksidb->update($this->input->post('id_sikad_keu_koreksi'));
          }else{
            //$this->siakad_keu_koreksidb->save();
            $this->siakad_keu_koreksidb->saveas();
          }
    ...
    public function delete(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id'])){
                $this->siakad_keu_koreksidb->delete($this->input->post('id'));
    ...
    public function delete_detail(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id_sikad_keu_koreksi'])){
                $this->siakad_keu_koreksidb->upddel_detail($this->input->post('id_sikad_keu_koreksi'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
     ...
     public function delete_detailxx(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id_sikad_keu_koreksi'])){
                $this->siakad_keu_koreksidb->delete_detail($this->input->post('id_sikad_keu_koreksi'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
            }else {
                $this->session->set_flashdata('notif', 'Failed! No Data Deleted');
            }
    ...
    private function gen_faktur(){
        $last=$this->siakad_keu_koreksidb->get_last_pt();
        // print_r($last);
        if(!empty($last)):
...
/** Module siakad_keu_koreksi Controller **/

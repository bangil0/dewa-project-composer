...
class siakad_keu_paket extends MX_Controller {

    function __construct() {
        parent::__construct();
          
        //Load IgnitedDatatables Library
        $this->load->model('siakad_keu_paket_model','siakad_keu_paketdb',TRUE);
        $this->session->set_userdata('lihat','siakad_keu_paket');
    ...
    function __getnewfaktur(){
        ...
        $null=$this->siakad_keu_paketdb->ceknomornull();
        // print_r($null);
        if($null!=null||!empty($null)){
            ...
            $faktur=$this->siakad_keu_paketdb->genfaktur();
    ...
    public function getdatatables(){
        // if($this->isadmin()==1):
            ...
            $this->datatables->add_column('edit',"<div class='btn-group'>
                <a data-toggle='modal' href='#modal-id' data-load-remote='".base_url('siakad_keu_paket/getone/$1/')."' data-remote-target='#modal-id .modal-body' class='btn btn-info btn-xs'><i class='fa fa-info-circle'></i> </a>
    ...
    function enkrip(){
        ...
        // echo $this->session->userdata('siakad_keu_paket');
    }
    function isadmin(){
    ...
    function forms(){   
        ...
        $this->load->view('siakad_keu_paket_form_inside');
           
    }

    public function get($id_siakad_keu_paket=null){
        if($id_siakad_keu_paket!==null){
            echo json_encode($this->siakad_keu_paketdb->get_one($id_siakad_keu_paket));
        }
    }
    function tables(){
        $this->load->view('siakad_keu_paket_data');
    }

    function getone($id=null){
        if($id!==null){
            $data=$this->siakad_keu_paketdb->get_one($id);
            $jml=count($data);
            // print_r($jml);
            // print_r($data);
            $div='';
    ...
    public function submit(){
        if ($this->input->post('ajax')){
          if ($this->input->post('id_siakad_keu_paket')){
            $this->siakad_keu_paketdb->update($this->input->post('id_siakad_keu_paket'));
          }else{
            //$this->siakad_keu_paketdb->save();
            $this->siakad_keu_paketdb->saveas();
          }
    ...
    public function delete(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id'])){
                $this->siakad_keu_paketdb->delete($this->input->post('id'));
    ...
    public function delete_detail(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id_siakad_keu_paket'])){
                $this->siakad_keu_paketdb->upddel_detail($this->input->post('id_siakad_keu_paket'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
     ...
     public function delete_detailxx(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id_siakad_keu_paket'])){
                $this->siakad_keu_paketdb->delete_detail($this->input->post('id_siakad_keu_paket'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
            }else {
                $this->session->set_flashdata('notif', 'Failed! No Data Deleted');
            }
    ...
    private function gen_faktur(){
        $last=$this->siakad_keu_paketdb->get_last_pt();
        // print_r($last);
        if(!empty($last)):
...
/** Module siakad_keu_paket Controller **/

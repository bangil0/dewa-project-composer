...
class siakad_keu_master extends MX_Controller {

    function __construct() {
        parent::__construct();
          
        //Load IgnitedDatatables Library
        $this->load->model('siakad_keu_master_model','siakad_keu_masterdb',TRUE);
        $this->session->set_userdata('lihat','siakad_keu_master');
    ...
    function __getnewfaktur(){
        ...
        $null=$this->siakad_keu_masterdb->ceknomornull();
        // print_r($null);
        if($null!=null||!empty($null)){
            ...
            $faktur=$this->siakad_keu_masterdb->genfaktur();
    ...
    function enkrip(){
        ...
        // echo $this->session->userdata('siakad_keu_master');
    }
    function isadmin(){
    ...
    function forms(){   
        ...
        $this->load->view('siakad_keu_master_form_inside');
           
    }

    public function get($id_siakad_keu_master=null){
        if($id_siakad_keu_master!==null){
            echo json_encode($this->siakad_keu_masterdb->get_one($id_siakad_keu_master));
        }
    }
    function tables(){
        $this->load->view('siakad_keu_master_data');
    }

    function getone($id=null){
        if($id!==null){
            $data=$this->siakad_keu_masterdb->get_one($id);
            $jml=count($data);
            // print_r($jml);
            // print_r($data);
            $div='';
    ...
    public function submit(){
        if ($this->input->post('ajax')){
          if ($this->input->post('id_siakad_keu_master')){
            $this->siakad_keu_masterdb->update($this->input->post('id_siakad_keu_master'));
          }else{
            //$this->siakad_keu_masterdb->save();
            $this->siakad_keu_masterdb->saveas();
          }
    ...
    public function delete(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id'])){
                $this->siakad_keu_masterdb->delete($this->input->post('id'));
    ...
    public function delete_detail(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id_siakad_keu_master'])){
                $this->siakad_keu_masterdb->upddel_detail($this->input->post('id_siakad_keu_master'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
     ...
     public function delete_detailxx(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id_siakad_keu_master'])){
                $this->siakad_keu_masterdb->delete_detail($this->input->post('id_siakad_keu_master'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
            }else {
                $this->session->set_flashdata('notif', 'Failed! No Data Deleted');
            }
    ...
    private function gen_faktur(){
        $last=$this->siakad_keu_masterdb->get_last_pt();
        // print_r($last);
        if(!empty($last)):
...
/** Module siakad_keu_master Controller **/

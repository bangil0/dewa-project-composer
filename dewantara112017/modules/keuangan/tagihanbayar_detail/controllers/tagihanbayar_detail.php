...
class tagihanbayar_detail extends MX_Controller {

    function __construct() {
        parent::__construct();
          
        //Load IgnitedDatatables Library
        $this->load->model('tagihanbayar_detail_model','tagihanbayar_detaildb',TRUE);
        $this->session->set_userdata('lihat','tagihanbayar_detail');
    ...
    public function index() {
        $this->template->set_title('Kelola Tagihanbayar_detail');
        $this->template->add_js('var baseurl="'.base_url().'tagihanbayar_detail/";','embed');  
        $this->template->load_view('tagihanbayar_detail_view',array(
            'view'=>'',
            'title'=>'Kelola Data Tagihanbayar_detail',
            'subtitle'=>'Pengelolaan Tagihanbayar_detail',
            'breadcrumb'=>array(
            'Tagihanbayar_detail'),
        ));
    }
    ...
    function __getnewfaktur(){
        ...
        $null=$this->tagihanbayar_detaildb->ceknomornull();
        // print_r($null);
        if($null!=null||!empty($null)){
            ...
            $faktur=$this->tagihanbayar_detaildb->genfaktur();
    ...
    function __submitnomor($data){
     ...
     function __updatestatproses($faktur){
        ...
        $this->db->update('tagihanbayar_detail',$data);
    }
    ...
    public function getdatatables(){
        if($this->isadmin()==1):
            $this->datatables->select('id,kode,invoice,idpaket,idtarif,nominal_tarif,nominal_bayar,isvalidasi,tglvalidasi,isactive,islocked,isdeleted,datedeleted,userid,datetime,')
                            ->from('tagihanbayar_detail');
            $this->datatables->add_column('edit',"<div class='btn-group'>
                <a data-toggle='modal' href='#modal-id' data-load-remote='".base_url('tagihanbayar_detail/getone/$1/')."' data-remote-target='#modal-id .modal-body' class='btn btn-info btn-xs'><i class='fa fa-info-circle'></i> </a>
            ...
            $this->datatables->select('id,kode,invoice,idpaket,idtarif,nominal_tarif,nominal_bayar,isvalidasi,tglvalidasi,isactive,islocked,isdeleted,datedeleted,userid,datetime,')
                            ->from('tagihanbayar_detail');
            $this->datatables->add_column('edit',"<div class='btn-group'>
                <a data-toggle='modal' href='#modal-id' data-load-remote='".base_url('tagihanbayar_detail/getone/$1/')."' data-remote-target='#modal-id .modal-body' class='btn btn-info btn-xs'><i class='fa fa-info-circle'></i> </a></div>" , 'id');
            $this->datatables->unset_column('id');
        endif;
    ...
    function enkrip(){
        ...
        // echo $this->session->userdata('tagihanbayar_detail');
    }
    function isadmin(){
    ...
    function forms(){   
        ...
        $this->load->view('tagihanbayar_detail_form_inside');
           
    }

    public function get($id=null){
        if($id!==null){
            echo json_encode($this->tagihanbayar_detaildb->get_one($id));
        }
    }
    function tables(){
        $this->load->view('tagihanbayar_detail_data');
    }

    function getone($id=null){
        if($id!==null){
            $data=$this->tagihanbayar_detaildb->get_one($id);
            $jml=count($data);
            // print_r($jml);
            // print_r($data);
            $div='';
    ...
    public function submit(){
        if ($this->input->post('ajax')){
          if ($this->input->post('id')){
            $this->tagihanbayar_detaildb->update($this->input->post('id'));
          }else{
            //$this->tagihanbayar_detaildb->save();
            $this->tagihanbayar_detaildb->saveas();
          }
    ...
    public function delete(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id'])){
                $this->tagihanbayar_detaildb->delete($this->input->post('id'));
    ...
    public function delete_detail(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id'])){
                $this->tagihanbayar_detaildb->upddel_detail($this->input->post('id'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
     ...
     public function delete_detailxx(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id'])){
                $this->tagihanbayar_detaildb->delete_detail($this->input->post('id'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
            }else {
                $this->session->set_flashdata('notif', 'Failed! No Data Deleted');
            }
    ...
    private function gen_faktur(){
        $last=$this->tagihanbayar_detaildb->get_last_pt();
        // print_r($last);
        if(!empty($last)):
...
/** Module tagihanbayar_detail Controller **/

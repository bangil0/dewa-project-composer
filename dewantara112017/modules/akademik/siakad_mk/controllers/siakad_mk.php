...
class siakad_mk extends MX_Controller {

    function __construct() {
        parent::__construct();
          
        //Load IgnitedDatatables Library
        $this->load->model('siakad_mk_model','siakad_mkdb',TRUE);
        $this->session->set_userdata('lihat','siakad_mk');
    ...
    public function index() {
        $this->template->set_title('Kelola Siakad_mk');
        $this->template->add_js('var baseurl="'.base_url().'siakad_mk/";','embed');  
        $this->template->load_view('siakad_mk_view',array(
            'view'=>'',
            'title'=>'Kelola Data Siakad_mk',
            'subtitle'=>'Pengelolaan Siakad_mk',
            'breadcrumb'=>array(
            'Siakad_mk'),
        ));
    }
    ...
    function __getnewfaktur(){
        ...
        $null=$this->siakad_mkdb->ceknomornull();
        // print_r($null);
        if($null!=null||!empty($null)){
            ...
            $faktur=$this->siakad_mkdb->genfaktur();
    ...
    public function getdatatables(){
        if($this->isadmin()==1):
            $this->datatables->select('kode_mk,id_siakad_kurikulum,kode_prodi,nm_mk,jns_mk,kurikulum_mk,kelompok_mk,sks_mk,sks_tatapmuka,sks_praktikum,min_mk_lulus,status_mk,upload_silabus_mk,upload_sap_mk,upload_bahan_mk,upload_diktat_mk,')
                            ->from('siakad_mk');
            $this->datatables->add_column('edit',"<div class='btn-group'>
                <a data-toggle='modal' href='#modal-id' data-load-remote='".base_url('siakad_mk/getone/$1/')."' data-remote-target='#modal-id .modal-body' class='btn btn-info btn-xs'><i class='fa fa-info-circle'></i> </a>

                <a href='#outside' data-toggle='tooltip' data-placement='top' title='Edit' class='edit btn btn-xs btn-success' id='$1'><i class='glyphicon glyphicon-edit'></i></a>
                <button data-toggle='tooltip' data-placement='top' title='Hapus' class='delete btn btn-xs btn-danger' id='$1'><i class='glyphicon glyphicon-remove'></i></button>
                </div>" , 'kode_mk');
            $this->datatables->unset_column('kode_mk');

        else:
            $this->datatables->select('kode_mk,id_siakad_kurikulum,kode_prodi,nm_mk,jns_mk,kurikulum_mk,kelompok_mk,sks_mk,sks_tatapmuka,sks_praktikum,min_mk_lulus,status_mk,upload_silabus_mk,upload_sap_mk,upload_bahan_mk,upload_diktat_mk,')
                            ->from('siakad_mk');
            $this->datatables->add_column('edit',"<div class='btn-group'>
                <a data-toggle='modal' href='#modal-id' data-load-remote='".base_url('siakad_mk/getone/$1/')."' data-remote-target='#modal-id .modal-body' class='btn btn-info btn-xs'><i class='fa fa-info-circle'></i> </a></div>" , 'kode_mk');
            $this->datatables->unset_column('kode_mk');
        endif;
    ...
    function enkrip(){
        ...
        // echo $this->session->userdata('siakad_mk');
    }
    function isadmin(){
    ...
    function forms(){   
        ...
        $this->load->view('siakad_mk_form_inside');
           
    }

    public function get($kode_mk=null){
        if($kode_mk!==null){
            echo json_encode($this->siakad_mkdb->get_one($kode_mk));
        }
    }
    function tables(){
        $this->load->view('siakad_mk_data');
    }

    function getone($id=null){
        if($id!==null){
            $data=$this->siakad_mkdb->get_one($id);
            $jml=count($data);
            // print_r($jml);
            // print_r($data);
            $div='';
    ...
    public function submit(){
        if ($this->input->post('ajax')){
          if ($this->input->post('kode_mk')){
            $this->siakad_mkdb->update($this->input->post('kode_mk'));
          }else{
            //$this->siakad_mkdb->save();
            $this->siakad_mkdb->saveas();
          }
    ...
    public function delete(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id'])){
                $this->siakad_mkdb->delete($this->input->post('id'));
    ...
    public function delete_detail(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['kode_mk'])){
                $this->siakad_mkdb->upddel_detail($this->input->post('kode_mk'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
     ...
     public function delete_detailxx(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['kode_mk'])){
                $this->siakad_mkdb->delete_detail($this->input->post('kode_mk'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
            }else {
                $this->session->set_flashdata('notif', 'Failed! No Data Deleted');
            }
    ...
    private function gen_faktur(){
        $last=$this->siakad_mkdb->get_last_pt();
        // print_r($last);
        if(!empty($last)):

...
class siakad_info extends MX_Controller {

    function __construct() {
        parent::__construct();
          
        //Load IgnitedDatatables Library
        $this->load->model('siakad_info_model','siakad_infodb',TRUE);
        $this->session->set_userdata('lihat','siakad_info');
    ...
    public function index() {
        $this->template->set_title('Kelola Siakad_info');
        $this->template->add_js('var baseurl="'.base_url().'siakad_info/";','embed');  
        $this->template->load_view('siakad_info_view',array(
            'view'=>'',
            'title'=>'Kelola Data Siakad_info',
            'subtitle'=>'Pengelolaan Siakad_info',
            'breadcrumb'=>array(
            'Siakad_info'),
        ));
    }
    ...
    function __getnewfaktur(){
        ...
        $null=$this->siakad_infodb->ceknomornull();
        // print_r($null);
        if($null!=null||!empty($null)){
            ...
            $faktur=$this->siakad_infodb->genfaktur();
    ...
    public function getdatatables(){
        if($this->isadmin()==1):
            $this->datatables->select('id_siakad_info,nm_info,ket_info,tgl_info,')
                            ->from('siakad_info');
            $this->datatables->add_column('edit',"<div class='btn-group'>
                <a data-toggle='modal' href='#modal-id' data-load-remote='".base_url('siakad_info/getone/$1/')."' data-remote-target='#modal-id .modal-body' class='btn btn-info btn-xs'><i class='fa fa-info-circle'></i> </a>

                <a href='#outside' data-toggle='tooltip' data-placement='top' title='Edit' class='edit btn btn-xs btn-success' id='$1'><i class='glyphicon glyphicon-edit'></i></a>
                <button data-toggle='tooltip' data-placement='top' title='Hapus' class='delete btn btn-xs btn-danger' id='$1'><i class='glyphicon glyphicon-remove'></i></button>
                </div>" , 'id_siakad_info');
            $this->datatables->unset_column('id_siakad_info');

        else:
            $this->datatables->select('id_siakad_info,nm_info,ket_info,tgl_info,')
                            ->from('siakad_info');
            $this->datatables->add_column('edit',"<div class='btn-group'>
                <a data-toggle='modal' href='#modal-id' data-load-remote='".base_url('siakad_info/getone/$1/')."' data-remote-target='#modal-id .modal-body' class='btn btn-info btn-xs'><i class='fa fa-info-circle'></i> </a></div>" , 'id_siakad_info');
            $this->datatables->unset_column('id_siakad_info');
        endif;
    ...
    function enkrip(){
        ...
        // echo $this->session->userdata('siakad_info');
    }
    function isadmin(){
    ...
    function forms(){   
        ...
        $this->load->view('siakad_info_form_inside');
           
    }

    public function get($id_siakad_info=null){
        if($id_siakad_info!==null){
            echo json_encode($this->siakad_infodb->get_one($id_siakad_info));
        }
    }
    function tables(){
        $this->load->view('siakad_info_data');
    }

    function getone($id=null){
        if($id!==null){
            $data=$this->siakad_infodb->get_one($id);
            $jml=count($data);
            // print_r($jml);
            // print_r($data);
            $div='';
    ...
    public function submit(){
        if ($this->input->post('ajax')){
          if ($this->input->post('id_siakad_info')){
            $this->siakad_infodb->update($this->input->post('id_siakad_info'));
          }else{
            //$this->siakad_infodb->save();
            $this->siakad_infodb->saveas();
          }
    ...
    public function delete(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id'])){
                $this->siakad_infodb->delete($this->input->post('id'));
    ...
    public function delete_detail(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id_siakad_info'])){
                $this->siakad_infodb->upddel_detail($this->input->post('id_siakad_info'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
     ...
     public function delete_detailxx(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id_siakad_info'])){
                $this->siakad_infodb->delete_detail($this->input->post('id_siakad_info'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
            }else {
                $this->session->set_flashdata('notif', 'Failed! No Data Deleted');
            }
    ...
    private function gen_faktur(){
        $last=$this->siakad_infodb->get_last_pt();
        // print_r($last);
        if(!empty($last)):

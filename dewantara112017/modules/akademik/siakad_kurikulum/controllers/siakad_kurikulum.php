...
class siakad_kurikulum extends MX_Controller {
    ...
    function __construct() {
        ...
        $this->session->set_userdata('lihat','siakad_kurikulum');
    ...
    public function index() {
        $this->template->set_title('Kelola Siakad_kurikulum');
        $this->template->add_js('var baseurl="'.base_url().'siakad_kurikulum/";','embed');  
        $this->template->load_view('siakad_kurikulum_view',array(
            'view'=>'',
            'title'=>'Kelola Data Siakad_kurikulum',
            'subtitle'=>'Pengelolaan Siakad_kurikulum',
            'breadcrumb'=>array(
            'Siakad_kurikulum'),
        ));
    }
    ...
    function __getnewfaktur(){
        ...
        $null=$this->siakad_kurikulumdb->ceknomornull();
        // print_r($null);
        if($null!=null||!empty($null)){
            ...
            $faktur=$this->siakad_kurikulumdb->genfaktur();
    ...
    public function getdatatables(){
        if($this->isadmin()==1):
            $this->datatables->select('id_siakad_kurikulum,kode_prodi,kode_kurikulum,nm_kurikulum,')
                            ->from('siakad_kurikulum');
            $this->datatables->add_column('edit',"<div class='btn-group'>
                <a data-toggle='modal' href='#modal-id' data-load-remote='".base_url('siakad_kurikulum/getone/$1/')."' data-remote-target='#modal-id .modal-body' class='btn btn-info btn-xs'><i class='fa fa-info-circle'></i> </a>

                <a href='#outside' data-toggle='tooltip' data-placement='top' title='Edit' class='edit btn btn-xs btn-success' id='$1'><i class='glyphicon glyphicon-edit'></i></a>
                <button data-toggle='tooltip' data-placement='top' title='Hapus' class='delete btn btn-xs btn-danger' id='$1'><i class='glyphicon glyphicon-remove'></i></button>
                </div>" , 'id_siakad_kurikulum');
            $this->datatables->unset_column('id_siakad_kurikulum');

        else:
            $this->datatables->select('id_siakad_kurikulum,kode_prodi,kode_kurikulum,nm_kurikulum,')
                            ->from('siakad_kurikulum');
            $this->datatables->add_column('edit',"<div class='btn-group'>
                <a data-toggle='modal' href='#modal-id' data-load-remote='".base_url('siakad_kurikulum/getone/$1/')."' data-remote-target='#modal-id .modal-body' class='btn btn-info btn-xs'><i class='fa fa-info-circle'></i> </a></div>" , 'id_siakad_kurikulum');
            $this->datatables->unset_column('id_siakad_kurikulum');
        endif;
    ...
    function enkrip(){
        ...
        // echo $this->session->userdata('siakad_kurikulum');
    }
    function isadmin(){
    ...
    function forms(){   
        ...
        $this->load->view('siakad_kurikulum_form_inside');
           
    }

    public function get($id_siakad_kurikulum=null){
        if($id_siakad_kurikulum!==null){
            echo json_encode($this->siakad_kurikulumdb->get_one($id_siakad_kurikulum));
        }
    }
    function tables(){
        $this->load->view('siakad_kurikulum_data');
    }

    function getone($id=null){
        if($id!==null){
            $data=$this->siakad_kurikulumdb->get_one($id);
            $jml=count($data);
            // print_r($jml);
            // print_r($data);
            $div='';
    ...
    public function submit(){
        if ($this->input->post('ajax')){
          if ($this->input->post('id_siakad_kurikulum')){
            $this->siakad_kurikulumdb->update($this->input->post('id_siakad_kurikulum'));
          }else{
            //$this->siakad_kurikulumdb->save();
            $this->siakad_kurikulumdb->saveas();
          }
    ...
    public function delete(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id'])){
                $this->siakad_kurikulumdb->delete($this->input->post('id'));
    ...
    public function delete_detail(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id_siakad_kurikulum'])){
                $this->siakad_kurikulumdb->upddel_detail($this->input->post('id_siakad_kurikulum'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
     ...
     public function delete_detailxx(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id_siakad_kurikulum'])){
                $this->siakad_kurikulumdb->delete_detail($this->input->post('id_siakad_kurikulum'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
            }else {
                $this->session->set_flashdata('notif', 'Failed! No Data Deleted');
            }
    ...
    private function gen_faktur(){
        $last=$this->siakad_kurikulumdb->get_last_pt();
        // print_r($last);
        if(!empty($last)):
...
/** Module siakad_kurikulum Controller **/

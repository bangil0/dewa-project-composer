...
class Siakad_sistem_model extends CI_Model {
    ...
    function get_all($limit, $uri) {
        ...
        $result = $this->db->get('siakad_sistem', $limit, $uri);
    ...
    function ceknomornull(){
        ...
        $this->db->order_by('id_siakad_sistem','ASC');
        $this->db->limit(1);

        $result=$this->db->get('siakad_sistem');
    ...
    function get_last(){
        ...
        $this->db->select('*'); //faktur
        $this->db->order_by('id_siakad_sistem','DESC');
        $this->db->limit(1);
    ...
    function get_one($id_siakad_sistem) {
        $this->db->where('id_siakad_sistem', $id_siakad_sistem);
        $result = $this->db->get('siakad_sistem');
        if ($result->num_rows() == 1) {
            return $result->row_array();
        } else {
    ...
    function __save(){
       ...
       $data = array(
            ...
            'smtr_akademik' => $this->input->post('smtr_akademik', TRUE),
           
            'tgl_start_akademik' => $this->input->post('tgl_start_akademik', TRUE),
           
            'tgl_end_akademik' => $this->input->post('tgl_end_akademik', TRUE),
    ...
    function savesiakad_sistem($data){
        $this->db->insert('siakad_sistem',$data);
    }
    function save_detail($data){
        $this->db->insert('siakad_sistem_detail',$data);
    }
    function upddel_detail($id_siakad_sistem=null) {
    ...
    function updatebyid($id_siakad_sistem,$data){
        $this->db->where('id_siakad_sistem', $id_siakad_sistem);
        $this->db->update('siakad_sistem', $data);
    }
    function update($id_siakad_sistem) {
        $data = array(
        'id_siakad_sistem' => $this->input->post('id_siakad_sistem',TRUE),
       'thn_akademik' => $this->input->post('thn_akademik', TRUE),
    ...
    function delete($id_siakad_sistem) {
        $this->db->where('id_siakad_sistem', $id_siakad_sistem);
        $this->db->delete('siakad_sistem'); 
       
    }
    function delete_detail($id=null) {
        $this->db->where('id_detail', $id);
        $this->db->delete('siakad_sistem_detail'); 
    ...
    function get_dropdown_array($value){
        $result = array();
        $array_keys_values = $this->db->query('select id_siakad_sistem, '.$value.' from siakad_sistem order by id_siakad_sistem asc');
        $result[0]="-- Pilih Urutan id_siakad_sistem --";
        foreach ($array_keys_values->result() as $row)
        {
            $result[$row->id_siakad_sistem]= $row->$value;
        }
        return $result;
    }

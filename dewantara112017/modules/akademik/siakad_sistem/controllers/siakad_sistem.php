...
class siakad_sistem extends MX_Controller {

    function __construct() {
        parent::__construct();
          
        //Load IgnitedDatatables Library
        $this->load->model('siakad_sistem_model','siakad_sistemdb',TRUE);
        $this->session->set_userdata('lihat','siakad_sistem');
    ...
    public function index() {
        $this->template->set_title('Kelola Siakad_sistem');
        $this->template->add_js('var baseurl="'.base_url().'siakad_sistem/";','embed');  
        $this->template->load_view('siakad_sistem_view',array(
            'view'=>'',
            'title'=>'Kelola Data Siakad_sistem',
            'subtitle'=>'Pengelolaan Siakad_sistem',
            'breadcrumb'=>array(
            'Siakad_sistem'),
        ));
    }
    ...
    function __getnewfaktur(){
        ...
        $null=$this->siakad_sistemdb->ceknomornull();
        // print_r($null);
        if($null!=null||!empty($null)){
            ...
            $faktur=$this->siakad_sistemdb->genfaktur();
    ...
    function __submitnomor($data){
       ...
       $this->db->insert('siakad_sistem',$data);
     ...
     function __updatestatproses($faktur){
        ...
        $this->db->update('siakad_sistem',$data);
    }
    ...
    public function getdatatables(){
        if($this->isadmin()==1):
            $this->datatables->select('id_siakad_sistem,thn_akademik,kode_akademik,status_akademik,smtr_akademik,tgl_start_akademik,tgl_end_akademik,')
                            ->from('siakad_sistem');
            $this->datatables->add_column('edit',"<div class='btn-group'>
                <a data-toggle='modal' href='#modal-id' data-load-remote='".base_url('siakad_sistem/getone/$1/')."' data-remote-target='#modal-id .modal-body' class='btn btn-info btn-xs'><i class='fa fa-info-circle'></i> </a>

                <a href='#outside' data-toggle='tooltip' data-placement='top' title='Edit' class='edit btn btn-xs btn-success' id='$1'><i class='glyphicon glyphicon-edit'></i></a>
                <button data-toggle='tooltip' data-placement='top' title='Hapus' class='delete btn btn-xs btn-danger' id='$1'><i class='glyphicon glyphicon-remove'></i></button>
                </div>" , 'id_siakad_sistem');
            $this->datatables->unset_column('id_siakad_sistem');

        else:
            $this->datatables->select('id_siakad_sistem,thn_akademik,kode_akademik,status_akademik,smtr_akademik,tgl_start_akademik,tgl_end_akademik,')
                            ->from('siakad_sistem');
            $this->datatables->add_column('edit',"<div class='btn-group'>
                <a data-toggle='modal' href='#modal-id' data-load-remote='".base_url('siakad_sistem/getone/$1/')."' data-remote-target='#modal-id .modal-body' class='btn btn-info btn-xs'><i class='fa fa-info-circle'></i> </a></div>" , 'id_siakad_sistem');
            $this->datatables->unset_column('id_siakad_sistem');
        endif;
    ...
    function enkrip(){
        ...
        // echo $this->session->userdata('siakad_sistem');
    }
    function isadmin(){
    ...
    function forms(){   
        ...
        $this->load->view('siakad_sistem_form_inside');
           
    }

    public function get($id_siakad_sistem=null){
        if($id_siakad_sistem!==null){
            echo json_encode($this->siakad_sistemdb->get_one($id_siakad_sistem));
        }
    }
    function tables(){
        $this->load->view('siakad_sistem_data');
    }

    function getone($id=null){
        if($id!==null){
            $data=$this->siakad_sistemdb->get_one($id);
            $jml=count($data);
            // print_r($jml);
            // print_r($data);
            $div='';
    ...
    public function submit(){
        if ($this->input->post('ajax')){
          if ($this->input->post('id_siakad_sistem')){
            $this->siakad_sistemdb->update($this->input->post('id_siakad_sistem'));
          }else{
            //$this->siakad_sistemdb->save();
            $this->siakad_sistemdb->saveas();
          }
    ...
    public function delete(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id'])){
                $this->siakad_sistemdb->delete($this->input->post('id'));
    ...
    public function delete_detail(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id_siakad_sistem'])){
                $this->siakad_sistemdb->upddel_detail($this->input->post('id_siakad_sistem'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
     ...
     public function delete_detailxx(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id_siakad_sistem'])){
                $this->siakad_sistemdb->delete_detail($this->input->post('id_siakad_sistem'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
            }else {
                $this->session->set_flashdata('notif', 'Failed! No Data Deleted');
            }
    ...
    private function gen_faktur(){
        $last=$this->siakad_sistemdb->get_last_pt();
        // print_r($last);
        if(!empty($last)):
...
/** Module siakad_sistem Controller **/

...
class Siakad_jadwal_model extends CI_Model {
    ...
    function get_all($limit, $uri) {
        ...
        $result = $this->db->get('siakad_jadwal', $limit, $uri);
    ...
    function ceknomornull(){
        ...
        $this->db->order_by('id_siakad_jadwal','ASC');
        $this->db->limit(1);

        $result=$this->db->get('siakad_jadwal');
    ...
    function get_last(){
        ...
        $this->db->select('*'); //faktur
        $this->db->order_by('id_siakad_jadwal','DESC');
        $this->db->limit(1);
    ...
    function get_one($id_siakad_jadwal) {
        $this->db->where('id_siakad_jadwal', $id_siakad_jadwal);
        $result = $this->db->get('siakad_jadwal');
        if ($result->num_rows() == 1) {
            return $result->row_array();
        } else {
    ...
    function __save(){
       ...
       $data = array(
            ...
            'hari_jadwal' => $this->input->post('hari_jadwal', TRUE),
            ...
            'kapasitas_ruang' => $this->input->post('kapasitas_ruang', TRUE),
           
            'tatap_muka' => $this->input->post('tatap_muka', TRUE),
           
            'inisial_kelas_jadwal' => $this->input->post('inisial_kelas_jadwal', TRUE),
    ...
    function savesiakad_jadwal($data){
        $this->db->insert('siakad_jadwal',$data);
    }
    function save_detail($data){
        $this->db->insert('siakad_jadwal_detail',$data);
    }
    function upddel_detail($id_siakad_jadwal=null) {
    ...
    function updatebyid($id_siakad_jadwal,$data){
        $this->db->where('id_siakad_jadwal', $id_siakad_jadwal);
        $this->db->update('siakad_jadwal', $data);
    }
    function update($id_siakad_jadwal) {
        $data = array(
    ...
    function delete($id_siakad_jadwal) {
        $this->db->where('id_siakad_jadwal', $id_siakad_jadwal);
        $this->db->delete('siakad_jadwal'); 
       
    }
    function delete_detail($id=null) {
        $this->db->where('id_detail', $id);
        $this->db->delete('siakad_jadwal_detail'); 
    ...
    function get_dropdown_array($value){
        $result = array();
        $array_keys_values = $this->db->query('select id_siakad_jadwal, '.$value.' from siakad_jadwal order by id_siakad_jadwal asc');
        $result[0]="-- Pilih Urutan id_siakad_jadwal --";
        foreach ($array_keys_values->result() as $row)
        {
            $result[$row->id_siakad_jadwal]= $row->$value;
        }
        return $result;
    }

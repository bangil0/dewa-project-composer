...
class Siakad_nilai_model extends CI_Model {
    ...
    function get_all($limit, $uri) {
        ...
        $result = $this->db->get('siakad_nilai', $limit, $uri);
    ...
    function ceknomornull(){
        ...
        $this->db->order_by('id_siakad_nilai','ASC');
        $this->db->limit(1);

        $result=$this->db->get('siakad_nilai');
    ...
    function get_last(){
        ...
        $this->db->select('*'); //faktur
        $this->db->order_by('id_siakad_nilai','DESC');
        $this->db->limit(1);
    ...
    function get_one($id_siakad_nilai) {
        $this->db->where('id_siakad_nilai', $id_siakad_nilai);
        $result = $this->db->get('siakad_nilai');
        if ($result->num_rows() == 1) {
            return $result->row_array();
        } else {
    ...
    function save() {
        $data=$this->__save();
        $this->db->insert('siakad_nilai', $data);
    }
    function saveas() {
        $data=$this->__saveas();
        $this->db->insert('siakad_nilai', $data);
    ...
    function __save(){
       ...
       $data = array(
            ...
            'bts_nilai_awal' => $this->input->post('bts_nilai_awal', TRUE),
           
            'bts_nilai_akhir' => $this->input->post('bts_nilai_akhir', TRUE),
    ...
    function savesiakad_nilai($data){
        $this->db->insert('siakad_nilai',$data);
    }
    function save_detail($data){
        $this->db->insert('siakad_nilai_detail',$data);
    }
    function upddel_detail($id_siakad_nilai=null) {
        ...
        $this->db->where('id_siakad_nilai', $id_siakad_nilai);
        $this->db->update('siakad_nilai', $data);
       
    } 
    function updatebyid($id_siakad_nilai,$data){
        $this->db->where('id_siakad_nilai', $id_siakad_nilai);
        $this->db->update('siakad_nilai', $data);
    }
    function update($id_siakad_nilai) {
        $data = array(
        'id_siakad_nilai' => $this->input->post('id_siakad_nilai',TRUE),
       'kode_akademik' => $this->input->post('kode_akademik', TRUE),
    ...
    function delete($id_siakad_nilai) {
        $this->db->where('id_siakad_nilai', $id_siakad_nilai);
        $this->db->delete('siakad_nilai'); 
       
    }
    function delete_detail($id=null) {
        $this->db->where('id_detail', $id);
        $this->db->delete('siakad_nilai_detail'); 
    ...
    function get_dropdown_array($value){
        $result = array();
        $array_keys_values = $this->db->query('select id_siakad_nilai, '.$value.' from siakad_nilai order by id_siakad_nilai asc');
        $result[0]="-- Pilih Urutan id_siakad_nilai --";
        foreach ($array_keys_values->result() as $row)
        {
            $result[$row->id_siakad_nilai]= $row->$value;
        }
        return $result;
    }

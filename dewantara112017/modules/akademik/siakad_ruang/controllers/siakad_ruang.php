...
class siakad_ruang extends MX_Controller {

    function __construct() {
        parent::__construct();
          
        //Load IgnitedDatatables Library
        $this->load->model('siakad_ruang_model','siakad_ruangdb',TRUE);
        $this->session->set_userdata('lihat','siakad_ruang');
    ...
    public function index() {
        $this->template->set_title('Kelola Siakad_ruang');
        $this->template->add_js('var baseurl="'.base_url().'siakad_ruang/";','embed');  
        $this->template->load_view('siakad_ruang_view',array(
            'view'=>'',
            'title'=>'Kelola Data Siakad_ruang',
            'subtitle'=>'Pengelolaan Siakad_ruang',
            'breadcrumb'=>array(
            'Siakad_ruang'),
        ));
    }
    ...
    function __getnewfaktur(){
        ...
        $null=$this->siakad_ruangdb->ceknomornull();
        // print_r($null);
        if($null!=null||!empty($null)){
            ...
            $faktur=$this->siakad_ruangdb->genfaktur();
    ...
    function __submitnomor($data){
       ...
       $this->db->insert('siakad_ruang',$data);
     ...
     function __updatestatproses($faktur){
        ...
        $this->db->update('siakad_ruang',$data);
    }
    ...
    public function getdatatables(){
        if($this->isadmin()==1):
            $this->datatables->select('id_siakad_ruang,inisial_ruangan,nm_ruangan,')
                            ->from('siakad_ruang');
            $this->datatables->add_column('edit',"<div class='btn-group'>
                <a data-toggle='modal' href='#modal-id' data-load-remote='".base_url('siakad_ruang/getone/$1/')."' data-remote-target='#modal-id .modal-body' class='btn btn-info btn-xs'><i class='fa fa-info-circle'></i> </a>

                <a href='#outside' data-toggle='tooltip' data-placement='top' title='Edit' class='edit btn btn-xs btn-success' id='$1'><i class='glyphicon glyphicon-edit'></i></a>
                <button data-toggle='tooltip' data-placement='top' title='Hapus' class='delete btn btn-xs btn-danger' id='$1'><i class='glyphicon glyphicon-remove'></i></button>
                </div>" , 'id_siakad_ruang');
            $this->datatables->unset_column('id_siakad_ruang');

        else:
            $this->datatables->select('id_siakad_ruang,inisial_ruangan,nm_ruangan,')
                            ->from('siakad_ruang');
            $this->datatables->add_column('edit',"<div class='btn-group'>
                <a data-toggle='modal' href='#modal-id' data-load-remote='".base_url('siakad_ruang/getone/$1/')."' data-remote-target='#modal-id .modal-body' class='btn btn-info btn-xs'><i class='fa fa-info-circle'></i> </a></div>" , 'id_siakad_ruang');
            $this->datatables->unset_column('id_siakad_ruang');
        endif;
    ...
    function enkrip(){
        ...
        // echo $this->session->userdata('siakad_ruang');
    }
    function isadmin(){
    ...
    function forms(){   
        ...
        $this->load->view('siakad_ruang_form_inside');
           
    }

    public function get($id_siakad_ruang=null){
        if($id_siakad_ruang!==null){
            echo json_encode($this->siakad_ruangdb->get_one($id_siakad_ruang));
        }
    }
    function tables(){
        $this->load->view('siakad_ruang_data');
    }

    function getone($id=null){
        if($id!==null){
            $data=$this->siakad_ruangdb->get_one($id);
            $jml=count($data);
            // print_r($jml);
            // print_r($data);
            $div='';
    ...
    public function submit(){
        if ($this->input->post('ajax')){
          if ($this->input->post('id_siakad_ruang')){
            $this->siakad_ruangdb->update($this->input->post('id_siakad_ruang'));
          }else{
            //$this->siakad_ruangdb->save();
            $this->siakad_ruangdb->saveas();
          }
    ...
    public function delete(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id'])){
                $this->siakad_ruangdb->delete($this->input->post('id'));
    ...
    public function delete_detail(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id_siakad_ruang'])){
                $this->siakad_ruangdb->upddel_detail($this->input->post('id_siakad_ruang'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
     ...
     public function delete_detailxx(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id_siakad_ruang'])){
                $this->siakad_ruangdb->delete_detail($this->input->post('id_siakad_ruang'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
            }else {
                $this->session->set_flashdata('notif', 'Failed! No Data Deleted');
            }
    ...
    private function gen_faktur(){
        $last=$this->siakad_ruangdb->get_last_pt();
        // print_r($last);
        if(!empty($last)):
...
/** Module siakad_ruang Controller **/

...
class Siakad_ruang_model extends CI_Model {
    ...
    function get_all($limit, $uri) {
        ...
        $result = $this->db->get('siakad_ruang', $limit, $uri);
    ...
    function ceknomornull(){
        ...
        $this->db->order_by('id_siakad_ruang','ASC');
        $this->db->limit(1);

        $result=$this->db->get('siakad_ruang');
    ...
    function get_last(){
        ...
        $this->db->select('*'); //faktur
        $this->db->order_by('id_siakad_ruang','DESC');
        $this->db->limit(1);
    ...
    function get_one($id_siakad_ruang) {
        $this->db->where('id_siakad_ruang', $id_siakad_ruang);
        $result = $this->db->get('siakad_ruang');
        if ($result->num_rows() == 1) {
            return $result->row_array();
        } else {
    ...
    function __save(){
       ...
       $data = array(
            ...
            'inisial_ruangan' => $this->input->post('inisial_ruangan', TRUE),
    ...
    function savesiakad_ruang($data){
        $this->db->insert('siakad_ruang',$data);
    }
    function save_detail($data){
        $this->db->insert('siakad_ruang_detail',$data);
    }
    function upddel_detail($id_siakad_ruang=null) {
    ...
    function updatebyid($id_siakad_ruang,$data){
        $this->db->where('id_siakad_ruang', $id_siakad_ruang);
        $this->db->update('siakad_ruang', $data);
    }
    function update($id_siakad_ruang) {
        $data = array(
    ...
    function delete($id_siakad_ruang) {
        $this->db->where('id_siakad_ruang', $id_siakad_ruang);
        $this->db->delete('siakad_ruang'); 
       
    }
    function delete_detail($id=null) {
        $this->db->where('id_detail', $id);
        $this->db->delete('siakad_ruang_detail'); 
    ...
    function get_dropdown_array($value){
        $result = array();
        $array_keys_values = $this->db->query('select id_siakad_ruang, '.$value.' from siakad_ruang order by id_siakad_ruang asc');
        $result[0]="-- Pilih Urutan id_siakad_ruang --";
        foreach ($array_keys_values->result() as $row)
        {
            $result[$row->id_siakad_ruang]= $row->$value;
        }
        return $result;
    }
